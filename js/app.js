...
function renderSmartInsights() {
    ...
    container.innerHTML = `
        ...
        <div class="insight-card col-span-2 sm:col-span-2 bg-rose-500/10 border border-rose-500/20 rounded-xl p-3 flex items-center justify-between">
            ...
            <i data-lucide="alert-circle" class="w-6 h-6 text-rose-400 opacity-50 shrink-0"></i>
        </div>
    `;
    lucide.createIcons({ root: container });
}
...
function renderRecentSessions() {
    ...
    container.innerHTML = recent.map(session => `
        <div class="session-item flex items-center justify-between bg-white/5 border border-white/10 rounded-xl p-3 min-h-[44px] group">
            <div>
                <div class="font-bold text-base text-white group-hover:text-orange-400 transition-colors">${escapeHTML(session.subjectName)}</div>
                <div class="text-[11px] text-white/50">${session.date}</div>
            </div>
            <div class="flex items-center gap-3">
                <span class="text-sm font-bold text-emerald-400 bg-emerald-500/10 px-3 py-1 rounded-md">${formatStudyTimeShort(session.minutes)}</span>
                <button onclick="deleteSession(${session.subjectId}, '${session.id}')" aria-label="حذف الجلسة" class="w-11 h-11 flex items-center justify-center text-white/30 hover:text-red-400 transition-all hover:scale-110 btn-press opacity-50 group-hover:opacity-100" title="حذف الجلسة">
